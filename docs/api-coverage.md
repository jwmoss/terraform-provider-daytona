...
| Runners | `daytona_runner`, `daytona_runner`, `daytona_runner_full`, `daytona_runner_for_sandbox`, `daytona_runners`, and `daytona_runners_by_snapshot_ref` cover runner registration, read/list, deletion, full runner details, sandbox-to-runner lookup, and snapshot-ref runner mappings. Managed-service `/api/runners` currently returned `404 Cannot GET /api/runners` during live verification and needs Daytona-side route/account confirmation. |
...
| Runner self-service and job queue APIs | Authenticated-runner info, runner sandbox assignment, runner healthcheck, job polling, and job status updates are runner-agent protocol endpoints, not user-managed Terraform infrastructure. User-facing full runner details and sandbox/snapshot-ref runner lookups are covered by read-only data sources. |
