...
| Sandbox observability streams | Sandbox logs, traces, trace spans, and metrics can be added as bounded data sources later. They are high-volume operational telemetry, so this provider does not expose them until query bounds and state-size behavior are designed. |
